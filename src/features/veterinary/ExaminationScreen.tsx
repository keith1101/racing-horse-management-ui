...
import type { AnatomicalRegion, Examination, InjurySeverity } from './medicalData';

const Injury3DViewer = lazy(() => import('./Injury3DViewer').then(({ Injury3DViewer: component }) => ({ default: component })));

export function ExaminationScreen() {
  const { route, navigate, toast, horses, getHorse, can, recordExamination, isLocked, lockTraining } = useRtms();
  ...
  const [findings, setFindings] = useState<string[]>(['Heart rhythm regular and sound', 'Clear respiratory sounds bilaterally']);
  ...
  const [injuryRegion, setInjuryRegion] = useState<AnatomicalRegion>('Left Front Leg');
  ...
  const [recommendation, setRecommendation] = useState('Maintain current training program and hydration regimen.');
  ...
  const handleSave = () => {
    ...
    if (applyRestriction && !isLocked(selectedHorse.id)) {
      lockTraining(selectedHorse.id, {
        reason: `${injuryType} — ${injuryRegion}`,
  ...
  return (
    <Screen
      ...
      context={
        <button onClick={handleCancel} className="inline-flex items-center gap-1 text-[13px] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:underline">
          <Icon name="arrow-left" size={13} /> Back to veterinary
        </button>
      }
      primary={can('medical.record') ? <Button variant="primary" icon="check" onClick={handleSave}>Save examination</Button> : undefined}
    >
      <div className="mx-auto max-w-4xl flex flex-col gap-4 pb-8">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-4">
            {selectedHorse ? (
              <div className="flex items-center gap-3">
                <HorseAvatar name={selectedHorse.name} image={selectedHorse.image} size={48} rounded="md" />
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[15px] font-semibold text-[var(--color-text-primary)]">{selectedHorse.name}</span>
                    <HealthBadge status={selectedHorse.health} size="sm" />
                  </div>
                  <div className="text-[12px] text-[var(--color-text-secondary)] mt-0.5">
                    {selectedHorse.stable} · Stall {selectedHorse.stall} · {selectedHorse.breed} · {selectedHorse.ageYears} yrs {selectedHorse.sex}
                  </div>
                </div>
              </div>
            ) : null}
            <div className="w-full sm:w-auto">
              <Select
                ...
                value={horseId}
                onChange={setHorseId}
                options={horses.map((h) => ({ value: h.id, label: `${h.name} (${h.stable})` }))}
              />
            </div>
          </div>
        </Panel>
        ...
        <Panel padded>
          ...
          <div className="mt-4">
            <Suspense fallback={<div className="rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-6 text-sm text-[var(--color-text-secondary)]">Loading injury viewer…</div>}>
              <Injury3DViewer
        ...
        <Panel padded>
          <div className="flex items-center gap-1.5 text-[var(--color-text-primary)]">
            <Icon name="user" size={14} className="text-[var(--color-text-muted)]" />
          ...
          <div className="mt-3 flex gap-2">
            ...
            />
            <Button variant="secondary" icon="plus" onClick={handleAddSymptom}>Add</Button>
          </div>

          <div className="mt-3 space-y-1.5">
            {symptoms.length === 0 ? (
              <p className="text-[12px] text-[var(--color-text-muted)] italic">No symptoms reported by handlers.</p>
            ) : (
              symptoms.map((s, idx) => (
                <div key={idx} className="flex items-center justify-between gap-2 rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] px-3 py-1.5">
                  <div className="flex items-center gap-2">
                    <Icon name="user" size={13} className="text-[var(--color-text-muted)]" />
                    <span className="text-[13px] text-[var(--color-text-primary)]">{s}</span>
                  </div>
                  <button onClick={() => handleRemoveSymptom(idx)} className="text-[var(--color-text-muted)] hover:text-[var(--color-danger)] outline-none">
                    <Icon name="x" size={13} />
                  </button>
                </div>
              ))
        ...
        <Panel padded>
          <div className="flex items-center gap-1.5 text-[var(--color-text-primary)]">
            <Icon name="stethoscope" size={14} className="text-[var(--color-primary)]" />
            <SectionTitle>Clinical findings (professional evaluation)</SectionTitle>
          </div>
          <div className="mt-3 flex gap-2">
            <input
              type="text"
              ...
              className="h-9 flex-1 rounded-[var(--radius-xs)] border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-2.5 text-[13px] text-[var(--color-text-primary)] outline-none focus-visible:border-[var(--color-focus)] focus-visible:ring-2 focus-visible:ring-[var(--color-focus)]/30"
            />
            <Button variant="secondary" icon="plus" onClick={handleAddFinding}>Add</Button>
          </div>

          <div className="mt-3 space-y-1.5">
            {findings.length === 0 ? (
              <p className="text-[12px] text-[var(--color-text-muted)] italic">No findings recorded yet.</p>
            ) : (
              findings.map((f, idx) => (
                <div key={idx} className="flex items-center justify-between gap-2 rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] px-3 py-1.5">
                  <div className="flex items-center gap-2">
                    <Icon name="stethoscope" size={13} className="text-[var(--color-primary)]" />
                    <span className="text-[13px] text-[var(--color-text-primary)]">{f}</span>
                  </div>
                  <button onClick={() => handleRemoveFinding(idx)} className="text-[var(--color-text-muted)] hover:text-[var(--color-danger)] outline-none">
                    <Icon name="x" size={13} />
                  </button>
                </div>
              ))
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div>
              ...
              <div className="relative mt-1">
                ...
                />
                <span className="pointer-events-none absolute right-2.5 top-2 text-[12px] text-[var(--color-text-muted)]">°C</span>
              </div>
            </div>
            <div>
              ...
              <div className="relative mt-1">
                ...
                />
                <span className="pointer-events-none absolute right-2.5 top-2 text-[12px] text-[var(--color-text-muted)]">bpm</span>
              </div>
            </div>
            <div>
              ...
              <div className="relative mt-1">
                <input
                  ...
                  className="font-metric h-9 w-full rounded-[var(--radius-xs)] border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-2.5 pr-11 text-[13px] outline-none focus-visible:border-[var(--color-focus)] focus-visible:ring-2 focus-visible:ring-[var(--color-focus)]/30"
                />
                <span className="pointer-events-none absolute right-2.5 top-2 text-[12px] text-[var(--color-text-muted)]">/min</span>
              </div>
            </div>
            <div>
              ...
              <div className="relative mt-1">
                <input
                  ...
                  className="font-metric h-9 w-full rounded-[var(--radius-xs)] border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-2.5 pr-8 text-[13px] outline-none focus-visible:border-[var(--color-focus)] focus-visible:ring-2 focus-visible:ring-[var(--color-focus)]/30"
                />
                <span className="pointer-events-none absolute right-2.5 top-2 text-[12px] text-[var(--color-text-muted)]">kg</span>
              </div>
            </div>
          </div>
        </Panel>
